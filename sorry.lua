...
function ValidateKey(key, serviceId, hwid)
	if not requestFunc then
		warn("[Pelinda Ov2.5] No supported request function found in this executor.")
	...
	if success and response then
		if response.StatusCode == 200 then
			...
			warn("[Pelinda Ov2.5] HTTP request was not successful. Code: " .. tostring(response.StatusCode))
			return false, "HTTP request failed"
		end
	else
		warn("[Pelinda Ov2.5] Request function pcall failed: " .. tostring(response))
...
local C = g("Submit", "arrow-right", function()
    ...
    isValidKey = ValidateKey(tostring(inputKey), "rohub", gethwid())
...
if k.KeySystem then
    ...
    local function validateRemoteKey()
        local success, result, reason = ValidateKey(q, "rohub", gethwid())
        if success then
            validated = true
        else
            warn("[KeySystem] Remote validation failed: " .. tostring(reason))
